...
public class EncoderHandler extends ChannelOutboundHandlerAdapter {
    ...
    private void readVersion() throws IOException {
        ...
        while (resources.hasMoreElements()) {
            try {
                ...
                if (name != null && name.equals("netty-com.corundumstudio.com.corundumstudio.socketio")) {
    ...
    private void sendMessage(HttpMessage msg, Channel channel, ByteBuf out, String type, ChannelPromise promise, HttpResponseStatus status) {
        ...
        String userAgent = channel.attr(com.corundumstudio.socketio.handler.EncoderHandler.USER_AGENT).get();
    ...
    private void handleHTTP(OutPacketMessage msg, ChannelHandlerContext ctx, ChannelPromise promise) throws IOException {
        ...
        ByteBuf out = encoder.allocateBuffer(ctx.alloc());
        Boolean b64 = ctx.channel().attr(com.corundumstudio.socketio.handler.EncoderHandler.B64).get();
        if (b64 != null && b64) {
            Integer jsonpIndex = ctx.channel().attr(com.corundumstudio.socketio.handler.EncoderHandler.JSONP_INDEX).get();
